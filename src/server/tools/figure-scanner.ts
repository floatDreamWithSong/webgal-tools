...
export async function scanStaticFigures(): Promise<string[]> {
  ...
  for (const figureDir of figureDirs) {
    ...
    await scanDirectoryForImages(fullDir, imageExtensions, staticFigures, figureDir);
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isFile()) {
        ...
        await scanDirectoryForImages(fullPath, extensions, results, baseDir);
      }
    }
  } catch (error) {
    // 目录不存在或无法访问，跳过
...
export async function scanLive2DFigures(): Promise<string[]> {
  ...
  for (const figureDir of figureDirs) {
    const fullDir = path.join(workDir, figureDir);
    await scanDirectoryForLive2DModels(fullDir, live2dModels, figureDir);
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isFile() && entry.name.endsWith('model.json')) {
        ...
        await scanDirectoryForLive2DModels(fullPath, results, baseDir);
      }
    }
  } catch (error) {
    // 目录不存在或无法访问，跳过
